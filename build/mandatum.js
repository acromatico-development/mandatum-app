function g(n){var t=new Date;return t.setDate(t.getDate()+n),t.toLocaleDateString("en-US",{day:"2-digit",month:"short",year:"2-digit"})}function c(n,t){var e=new Intl.NumberFormat("en-US",{style:"currency",currency:t});return e.format(n)}var p="stage-dot-mandatum-app.uc.r.appspot.com",v=class{constructor(t,e,s,a,o,d){this.container=t,this.loading=!0,this.shop=e,this.discount=s,this.days=a,this.productId=o,this.shopifyProduct=d,this.currency=d.variants[0].priceV2.currencyCode}async init(){return this.addStyles(),this.addMandatumButton(),await this.addMandatumModal(),this.loading=!1,this.loading}async addCartMandate(){let t=`gid://shopify/Product/${this.productId}`;console.log("Shopify Variant",this.shopifyVariant);try{let s=(await fetch(`https://${p}/getDiscountCode?shop=${this.shop}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId:t})}).then(l=>l.json())).codeDiscountNode.codeDiscount.codes.edges[0].node.code;console.log("Shopify Product",this.shopifyProduct);let o=[{variantId:this.shopifyProduct.variants.find(l=>l.title===this.shopifyVariant.title).id,quantity:1,customAttributes:[{key:"Mandatum Discount",value:`${this.discount}%`},{key:"Mandatum Delivery Days",value:`${this.days} days`}]}],d=[{key:"Mandatum Order",value:"true"}],i={price:"10.00",shippingRateHandle:"mandatum-shipping",title:"Mandatum Shipping"},u=(await fetch(`https://${p}/pay?shop=${this.shop}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({lineItems:o,customAttributes:d,shippingLine:i})}).then(l=>l.json())).draftOrder.invoiceUrl;console.log(u)}catch(e){console.log(e)}}toggleModal(){this.modalContainer.classList.toggle("open")}addStyles(){let t=document.querySelector("head"),e=document.createElement("style");e.innerHTML=`
      .mandatum-button {
        position: fixed;
        left: 50%;
        bottom: 2rem;
    ...
    `,t.appendChild(e)}async addMandatumModal(){let t=document.createElement("div"),e=await fetch(`${location.href.split("?")[0]}.json`).then(a=>a.json());t.classList.add("mandatum-modal"),t.innerHTML=`
      <div class="mandatum-modal-box">
        ...
        <p id="product-price-mandatum" class="product-price-mandatum">
          Price | <s>${c(this.shopifyProduct.variants[0].price,this.currency)}</s> <span>${c(this.shopifyProduct.variants[0].price*(1-this.discount/100),this.currency)}</span>
        </p>
        ...
        <div class="mandatum-badges">
          <div class="svg-badge">
            ...
            <p style="color: #767171">Discount:<br/>${c(this.shopifyProduct.variants[0].price*(this.discount/100),this.currency)}</p>
          </div>
          <div class="svg-badge">
            ...
            <p style="color: #548235">Donation:<br/>${c(this.shopifyProduct.variants[0].price*(this.discount/100),this.currency)}</p>
          </div>
          ...
          <div class="mandatum-info-box">
            ...
            <ol>
              ...
              <li>
                ...
                <ul>
                  <li>We apply a <span class="color">${c(this.shopifyProduct.variants[0].price*(this.discount/100),this.currency)} private discount</span> in the checkout process.</li>
                  <li>We <span class="color">donate ${c(this.shopifyProduct.variants[0].price*(this.discount/100),this.currency)}</span> to protect forests and oceans at no cost for you.</li>
                </ul>
              </li>
            </ol>
            <div class="mandatum-modal-buttons">
    ...
    `,this.container.appendChild(t),t.addEventListener("click",()=>{this.toggleModal()})}};async function f(){let n,t,e,s,a,o,d,i,m,u,l,h=location.pathname.includes("products");if(h?(console.log("Is Product"),t=document.querySelector("script[src*='mandatum']"),e=new URLSearchParams(t.src.split("?")[1]),s=e.get("shop"),a=document.querySelector("body"),o=await fetch(`${location.href.split("?")[0]}.json`).then(r=>r.json()),d=o.product.id,i=await fetch(`https://${p}/isMandatum?shop=${s}&product=${"gid://shopify/Product/"+d}`).then(r=>r.json()),m=i.isMandatum,u=parseFloat(i.descuento),l=parseInt(i.dias)):console.log("No Product"),m&&h){let r=i.newProduct;console.log("product",r),n=new v(a,s,u,l,d,r),n.init()}return n}f().then(n=>{window.mandatum=n});
