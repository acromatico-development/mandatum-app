function g(n){var t=new Date;return t.setDate(t.getDate()+n),t.toLocaleDateString("en-US",{day:"2-digit",month:"short",year:"2-digit"})}function c(n,t){var e=new Intl.NumberFormat("en-US",{style:"currency",currency:t});return e.format(n)}var p="stage-dot-mandatum-app.uc.r.appspot.com",v=class{constructor(t,e,s,a,o,d){this.container=t,this.loading=!0,this.shop=e,this.discount=s,this.days=a,this.productId=o,this.shopifyProduct=d.product,this.currency=d.shop.currencyCode}async init(){return this.addStyles(),this.addMandatumButton(),await this.addMandatumModal(),this.loading=!1,this.loading}async addCartMandate(){let t=`gid://shopify/Product/${this.productId}`;console.log("Shopify Variant",this.shopifyVariant);try{let s=(await fetch(`https://${p}/getDiscountCode?shop=${this.shop}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId:t})}).then(l=>l.json())).codeDiscountNode.codeDiscount.codes.edges[0].node.code;console.log("Shopify Product",this.shopifyProduct);let o=[{variantId:this.shopifyProduct.variants.edges.find(l=>l.node.title===this.shopifyVariant.title).id,quantity:1,customAttributes:[{key:"Mandatum Discount",value:`${this.discount}%`},{key:"Mandatum Delivery Days",value:`${this.days} days`}]}],d=[{key:"Mandatum Order",value:"true"}],i={price:"10.00",shippingRateHandle:"mandatum-shipping",title:"Mandatum Shipping"},r=await fetch(`https://${p}/pay?shop=${this.shop}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({lineItems:o,customAttributes:d,shippingLine:i})}).then(l=>l.json());console.log(r);let u=r.draftOrder.invoiceUrl;console.log(u)}catch(e){console.log(e)}}toggleModal(){this.modalContainer.classList.toggle("open")}addStyles(){let t=document.querySelector("head"),e=document.createElement("style");e.innerHTML=`
    ...
    `,this.container.appendChild(t),t.addEventListener("click",()=>{this.toggleModal()})}};async function f(){let n,t,e,s,a,o,d,i,r,u,l,h=location.pathname.includes("products");if(h?(console.log("Is Product"),t=document.querySelector("script[src*='mandatum']"),e=new URLSearchParams(t.src.split("?")[1]),s=e.get("shop"),a=document.querySelector("body"),o=await fetch(`${location.href.split("?")[0]}.json`).then(m=>m.json()),d=o.product.id,i=await fetch(`https://${p}/isMandatum?shop=${s}&product=${"gid://shopify/Product/"+d}`).then(m=>m.json()),r=i.isMandatum,u=parseFloat(i.descuento),l=parseInt(i.dias)):console.log("No Product"),r&&h){let m=i.newProduct;console.log("product",m),n=new v(a,s,u,l,d,m),n.init()}return n}f().then(n=>{window.mandatum=n});
