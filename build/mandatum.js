function g(n){var t=new Date;return t.setDate(t.getDate()+n),t.toLocaleDateString("en-US",{day:"2-digit",month:"short",year:"2-digit"})}function c(n,t){var o=new Intl.NumberFormat("en-US",{style:"currency",currency:t});return o.format(n)}var p="stage-dot-mandatum-app.uc.r.appspot.com",v=class{constructor(t,o,s,a,e,d){this.container=t,this.loading=!0,this.shop=o,this.discount=s,this.days=a,this.productId=e,this.shopifyProduct=d.product,this.currency=d.shop.currencyCode}async init(){return this.addStyles(),this.addMandatumButton(),await this.addMandatumModal(),this.loading=!1,this.loading}async addCartMandate(){let t=`gid://shopify/Product/${this.productId}`;console.log("Shopify Variant",this.shopifyVariant);try{let s=(await fetch(`https://${p}/getDiscountCode?shop=${this.shop}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId:t})}).then(l=>l.json())).codeDiscountNode.codeDiscount.codes.edges[0].node.code;console.log("Shopify Product",this.shopifyProduct);let e=[{variantId:this.shopifyProduct.variants.edges.find(l=>l.node.title===this.shopifyVariant.title).id,quantity:1,customAttributes:[{key:"Mandatum Discount",value:`${this.discount}%`},{key:"Mandatum Delivery Days",value:`${this.days} days`}]}],d=[{key:"Mandatum Order",value:"true"}],i={price:"10.00",shippingRateHandle:"mandatum-shipping",title:"Mandatum Shipping"},u=(await fetch(`https://${p}/pay?shop=${this.shop}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({lineItems:e,customAttributes:d,shippingLine:i})}).then(l=>l.json())).draftOrder.invoiceUrl;console.log(u)}catch(o){console.log(o)}}toggleModal(){this.modalContainer.classList.toggle("open")}addStyles(){let t=document.querySelector("head"),o=document.createElement("style");o.innerHTML=`
      .mandatum-button {
        position: fixed;
        left: 50%;
        bottom: 2rem;
    ...
    `,t.appendChild(o)}async addMandatumModal(){let t=document.createElement("div"),o=await fetch(`${location.href.split("?")[0]}.json`).then(a=>a.json());t.classList.add("mandatum-modal"),t.innerHTML=`
      <div class="mandatum-modal-box">
        ...
        <select id="product-select-mandatum" name="product-select-mandatum">
          ${this.shopifyProduct.variants.edges.reduce((a,e)=>{let d=`<option value="${e.node.id}">${e.node.title} - ${c(e.node.price,this.currency)}</option>`;return a+d},"")}
        </select>
        ...
        <div id="mandate_info_box" class="mandatum-info-modal">
          <div class="mandatum-info-box">
            ...
            <ol>
              <li>To be delivered by <span class="color">${g(this.days)}</span></li>
              <li>
    ...
    `,this.container.appendChild(t),t.addEventListener("click",()=>{this.toggleModal()})}};async function f(){let n,t,o,s,a,e,d,i,m,u,l,h=location.pathname.includes("products");if(h?(console.log("Is Product"),t=document.querySelector("script[src*='mandatum']"),o=new URLSearchParams(t.src.split("?")[1]),s=o.get("shop"),a=document.querySelector("body"),e=await fetch(`${location.href.split("?")[0]}.json`).then(r=>r.json()),d=e.product.id,i=await fetch(`https://${p}/isMandatum?shop=${s}&product=${"gid://shopify/Product/"+d}`).then(r=>r.json()),m=i.isMandatum,u=parseFloat(i.descuento),l=parseInt(i.dias)):console.log("No Product"),m&&h){let r=i.newProduct;console.log("product",r),n=new v(a,s,u,l,d,r),n.init()}return n}f().then(n=>{window.mandatum=n});
