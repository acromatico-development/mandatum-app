var u="mandatum-app.uc.r.appspot.com";function x(s){var t=new Date;return t.setDate(t.getDate()+s),t.toLocaleDateString("en-US",{day:"2-digit",month:"short",year:"2-digit"})}function c(s,t){var o=new Intl.NumberFormat("en-US",{style:"currency",currency:t});return o.format(s)}var v=class{constructor(t,o,d,e,a,n){this.container=t,this.loading=!0,this.shop=o,this.discount=d,this.days=e,this.productId=a,this.shopifyProduct=n.product,this.currency=n.shop.currencyCode}async init(){return this.addStyles(),this.addMandatumButton(),await this.addMandatumModal(),this.loading=!1,this.loading}async addCartMandate(){let t=`gid://shopify/Product/${this.productId}`;console.log("Shopify Variant",this.shopifyVariant);try{let d=(await fetch(`https://${u}/getDiscountCode?shop=${this.shop}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId:t})}).then(r=>r.json())).codeDiscountNode.codeDiscount.codes.edges[0].node.code;console.log("Shopify Product",this.shopifyProduct);let a=[{variantId:this.shopifyProduct.variants.edges.find(r=>r.node.title===this.shopifyVariant.title).node.id,quantity:1,customAttributes:[{key:"Mandatum Discount",value:`${this.discount}%`},{key:"Mandatum Delivery Days",value:`${this.days} days`}],appliedDiscount:{title:"Mandatum",description:d,value:this.discount,valueType:"PERCENTAGE"}}],n=[{key:"Mandatum Order",value:"true"}],m={price:"10.00",shippingRateHandle:"mandatum-shipping",title:"Mandatum Shipping"};console.log("LineItems: ",a);let l=await fetch(`https://${u}/pay?shop=${this.shop}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({lineItems:a,customAttributes:n,shippingLine:m})}).then(r=>r.json());console.log(l);let p=l.draftOrder.invoiceUrl;console.log(p),location.assign(p)}catch(o){console.log(o)}}toggleModal(){this.modalContainer.classList.toggle("open")}addStyles(){let t=document.querySelector("head"),o=document.createElement("style");o.innerHTML=`
    ...
    `,Shopify.Checkout.OrderStatus.addContentBox(o),t.appendChild(d),document.getElementById("go_to_mandatum").addEventListener("click",()=>{d.classList.add("open")}),document.getElementById("mandatum_modal_close").addEventListener("click",()=>{d.classList.remove("open")})}},k=y;async function z(){let s,t,o,d,e,a,n,m,l,p,r,h,g,w=location.pathname.includes("products"),C=location.pathname.includes("orders"),L=document.querySelector("script[src*='mandatum']"),f=new URLSearchParams(L.src.split("?")[1]).get("shop");if(w&&(console.log("Is Product"),o=document.querySelector("body"),d=await fetch(`${location.href.split("?")[0]}.json`).then(i=>i.json()),e=d.product.id,a=await fetch(`https://${u}/isMandatum?shop=${f}&product=${"gid://shopify/Product/"+e}`).then(i=>i.json()),n=a.isMandatum,m=parseFloat(a.descuento),l=parseInt(a.dias),p=a.newProduct.shop.privateMetafield.value!=="false"),p&&n&&w){let i=a.newProduct;console.log("product",i),s=new M(o,f,m,l,e,i),s.init()}if(C){console.log("Is Order");let i=Shopify.checkout.line_items.find(b=>!!b.properties["Mandatum Discount"]);r=!!i,h=parseFloat(i.properties["Mandatum Discount"].split("%")[0])/100,g=parseFloat(i.line_price)}if(C&&r){console.log("Is mandatum Order",h),console.log(g);let i=await fetch(`https://${u}/checkoutData?shop=${f}`).then(b=>b.json());t=new k(h,g,i.name),await t.init()}return{product:s,order:t}}z().then(s=>{window.mandatum=s});
